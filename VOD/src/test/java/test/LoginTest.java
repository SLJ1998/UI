...
public class LoginTest extends CommonTest {
    ...
    @DataProvider(name = "login")//给dataprovider获取到的数组取一个变量名
    ...
    @Test(dataProvider = "login")//名称为@DataProvider注解中获取到的二维数组的名称
    public void loginTest(HashMap<String, String> data) throws IOException {
        loginAction = new LoginAction(new CommonDriver().start());//初始化Action对象
        loginAction.login(data.get("username"), data.get("password"));//调用Action里的方法对获取到的元素进行操作
//        loginAction.login("admin", "123456a?admin","0");//调用Action里的方法对获取到的元素进行操作
        CommonTest.setDriver(loginAction.getDriver());//加载驱动
        intelligentWait.intelligentWait(loginAction.driver, 20, data.get("toassert"));//封装的智能等待
        Assert.assertTrue(loginAction.getDriver().getPageSource().contains(data.get("toassert")));//断言，实际结果与预期结果进行对比
    }
