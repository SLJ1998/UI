...
public class EnterApplicationTest extends CommonTest {
    public EnterApplicationAction enterApplicationAction;
    ...
    public Object[][] words() throws IOException {
        return excelData.getExcelData("src\\test\\java\\data\\ApplicationManager", "enter.xlsx", "sheet1");
    ...
    public void loginTest(HashMap<String, String> data) throws IOException {
        enterApplicationAction = new EnterApplicationAction(CommonDriver.getDriver());//初始化Action对象
        enterApplicationAction.enter();//调用Action里的方法对获取到的元素进行操作
//        loginAction.login("admin", "123456a?admin","0");//调用Action里的方法对获取到的元素进行操作
        CommonTest.setDriver(enterApplicationAction.getDriver());//加载驱动
        intelligentWait.intelligentWait(enterApplicationAction.driver, 20, data.get("toassert"));//封装的智能等待
        Assert.assertTrue(enterApplicationAction.getDriver().getPageSource().contains(data.get("toassert")));//断言，实际结果与预期结果进行对比
    }
