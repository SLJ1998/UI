...
import action.DistributionManagement.Refresh.QueryRefreshAction;
...
public class QueryRefreshTest extends CommonTest {
    public QueryRefreshAction queryRefreshAction;
    ...
    public Object[][] words() throws IOException {
        return excelData.getExcelData("src\\test\\java\\data\\DistributionManagement\\Refresh", "query.xlsx", "sheet1");
    ...
    public void loginTest(HashMap<String, String> data) throws IOException {
        queryRefreshAction = new QueryRefreshAction(CommonDriver.getDriver());//初始化Action对象
        queryRefreshAction.query(data.get("name"));//调用Action里的方法对获取到的元素进行操作
//        loginAction.login("admin", "123456a?admin","0");//调用Action里的方法对获取到的元素进行操作
        CommonTest.setDriver(queryRefreshAction.getDriver());//加载驱动
        intelligentWait.intelligentWait(queryRefreshAction.driver, 20, data.get("toassert"));//封装的智能等待
        Assert.assertTrue(queryRefreshAction.getDriver().getPageSource().contains(data.get("toassert")));//断言，实际结果与预期结果进行对比
    }
