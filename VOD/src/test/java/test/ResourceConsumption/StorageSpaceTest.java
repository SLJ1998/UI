...
import action.ResourceConsumption.StorageSpaceAction;
...
public class StorageSpaceTest extends CommonTest {
    public StorageSpaceAction storageSpaceAction;
    ...
    public void loginTest(HashMap<String, String> data) throws IOException {
        storageSpaceAction = new StorageSpaceAction(CommonDriver.getDriver());//初始化Action对象
        storageSpaceAction.storageSpace();//调用Action里的方法对获取到的元素进行操作
//        loginAction.login("admin", "123456a?admin","0");//调用Action里的方法对获取到的元素进行操作
        CommonTest.setDriver(storageSpaceAction.getDriver());//加载驱动
        intelligentWait.intelligentWait(storageSpaceAction.driver, 20, data.get("toassert"));//封装的智能等待
        Assert.assertTrue(storageSpaceAction.getDriver().getPageSource().contains(data.get("toassert")));//断言，实际结果与预期结果进行对比
    }
